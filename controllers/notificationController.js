...
exports.sendNotification = async (req, res) => {
  try {
    const { receivers, message, notificationType, actionDetails } = req.body;
    const sender = req.userId; // Get sender ID from authenticated user context
...
exports.scheduleNotification = async (req, res) => {
  try {
    const { receivers, message, notificationType, actionDetails, scheduledTime } = req.body;
    const sender = req.userId; // Get sender ID from authenticated user context
    ...
    const now = new Date();
    let scheduleTime = new Date(scheduledTime);

    if (scheduleTime <= now) {
      scheduleTime = new Date(now.getTime() + 1 * 60 * 1000);
    }
